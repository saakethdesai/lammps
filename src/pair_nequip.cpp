...
void PairNEQUIP::coeff(int narg, char **arg) {
  ...
  // edit 0: remove this check because I am introducing multiple models
  //if (narg != (3+ntypes))
  ...
  // edit 1: hardcode 7 models, check atom types after that
  // arg[3] onwards denotes elements, make that arg[9] onwards
  char **elements = new char*[ntypes+1];
  for (int i = 1; i <= ntypes; i++){
      elements[i] = new char [strlen(arg[i+8])+1];
      strcpy(elements[i], arg[i+8]);
  ...
  //edit 2: load series of models
  
  //unsure why the line below is wrong?
  //std::cout << "Loading models from " << arg[2] << " " << arg[3] < " " << arg[4] << " " << arg[5] << " " << arg[6] << " " < arg[7] << "\n";
  ...
  for (int i = 0; i < 7; i++)
  {
      //std::cout << arg[2+i] << std::endl;
      model = torch::jit::load(std::string(arg[2+i]), device, metadata);
      model.eval();
  ...
  //std::cout << "Read in " << model_list.size() << " models" << std::endl;
}
...
void PairNEQUIP::compute(int eflag, int vflag){
  ...
  */
  ...
  //reverse comm pair (edges)
  //reverse comm pair (e_vec)
  ...
  c10::Dict<std::string, torch::Tensor> outputs;
  ...
  std::vector<torch::IValue> output_vector(1, outputs);
  ...
  //edit 3: backward using full model, since it has autograd capabilities
  auto output_f_raw = model_list.back().forward(input_vector);
  //std::cout << "Converting " << std::endl;
  auto output_f = output_f_raw.toGenericDict();
  torch::Tensor forces_tensor = output_f.at("forces").toTensor().cpu();
  auto forces = forces_tensor.accessor<float, 2>();
  //std::cout << "Grabbed forces" << std::endl;
  
  //std::cout << "Entering model forward" << std::endl;
  //edit 4: forward on multiple models
  auto tmp_input = input_vector;
  //auto tmp_output = output_vector;
  for (auto it = model_list.begin(); it != model_list.end()-1; ++it)
  {
      auto tmp_output_dict = (*it).forward(tmp_input).toGenericDict();
      output_vector.at(0) = tmp_output_dict;
      //std::cout << "Current dict" << std::endl;
      //for (auto dict_it = tmp_output_dict.begin(); dict_it != tmp_output_dict.end(); ++dict_it)
      //{
      //    std::cout << "Name = " << dict_it->key() << " Shape = " << dict_it->value().toTensor().sizes() << std::endl;
      //}
      tmp_input = output_vector;
      //if ( (it != model_list.end()) && (next(it) != model_list.end()) )
      //    output_vector.pop_back();
      //std::cout << "Size = " << output_vector.size() << std::endl;
  }
  //inspect outputs
  //auto outputs = tmp_input.toTuple()->elements();
  //std::cout << "outputs: " << outputs << std::endl; 
  //std::cout << "Done model forward" << std::endl;

  auto output = output_vector.at(0).toGenericDict(); 
  //std::cout << "Stored last dictionary" << std::endl;
...
/* ---------------------------------------------------------------------- */
int PairNEQUIP::pack_forward_comm(int n, int *list, double *buf,
...
void PairNEQUIP::unpack_forward_comm(int n, int first, double *buf)
{
}
